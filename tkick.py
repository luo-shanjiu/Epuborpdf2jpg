...
fhc = Frame(root)
fhc.pack(side="left",expand=True)    #左块(按钮)

ffc = Frame(root)
ffc.pack(side="left",expand=True)    #右块（log）

text_box = Text(ffc, height=600, width=500)
text_box.pack(side=LEFT, fill=BOTH, expand=True)
scrollbar = Scrollbar(ffc, command=text_box.yview)
...
def pdf_to_jpg(pdf_path, output_folder, basename,dpix):
    try:
        ...
        showup(f"{basename}completed")
    except Exception as e:
        print(f"An error occurred: {e}")
        showup(f"An error occurred: {e}")
    ...
    #8.下沉至<html>
        ...
        for i in os.listdir(os.path.join(os.getcwd(), "image")):
            ...
            if i =="cover.jpg":
                ...
                showup(f"{basename}_cover.jpg completed")
                break
        ...
        for i in  range(len(jpgchick)):
            ...
            showup(f"{basename}_{i + 1}.jpg completed")                                 
...
def showupall():
    global text_box, pdfturn,epubturn,folderkik
    text_box.insert(END, f"pdfturn:{pdfturn}\n")
    text_box.insert(END, f"epubturn:{epubturn}\n")
    text_box.insert(END, f"folderkik:{folderkik}\n")

def change():
    global pdfturn,epubturn
    if len(pdfturn) != 0:
        for file in pdfturn:
...
Button1 = Button(fhc, text="show all", command=showupall)
Button1.pack()

button2 = Button(fhc, text="change", command=change)
button2.pack()
